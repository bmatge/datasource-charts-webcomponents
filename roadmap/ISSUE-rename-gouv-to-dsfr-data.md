...
Le prefixe actuel `gouv-*` / `gouv-widgets` ne reflete pas ce positionnement. Pour s'inscrire
...
- Package npm : `gouv-widgets` → `dsfr-data`
- Scope workspace : `@gouv-widgets/*` → `@dsfr-data/*`
...
| Actuel | Nouveau | Role |
|--------|---------|------|
| `gouv-source` | `dsfr-data-source` | Fetch de donnees (adapters API) |
| `gouv-query` | `dsfr-data-query` | Transformation (filter, group-by, aggregate, sort) |
| `gouv-normalize` | `dsfr-data-normalize` | Normalisation de donnees |
| `gouv-facets` | `dsfr-data-facets` | Filtres a facettes interactifs |
| `gouv-search` | `dsfr-data-search` | Recherche textuelle |
| `gouv-dsfr-chart` | `dsfr-data-chart` | Wrapper enrichi autour des composants dsfr-chart |
| `gouv-kpi` | `dsfr-data-kpi` | Indicateur cle de performance |
| `gouv-kpi-group` | `dsfr-data-kpi-group` | Groupe d'indicateurs |
| `gouv-datalist` | `dsfr-data-list` | Tableau / liste de donnees |
| `gouv-display` | `dsfr-data-display` | Affichage HTML libre |
| `gouv-world-map` | `dsfr-data-world-map` | Carte choropleth mondiale |
| `gouv-chart-a11y` | `dsfr-data-a11y` | Companion accessibilite (tableau, CSV, description) |
...
- [ ] Mettre a jour les noms de classes (`GouvSource` → `DsfrDataSource`, etc.)
...
- [ ] Renommer `package.json` name : `gouv-widgets` → `dsfr-data`
- [ ] Renommer tous les `@gouv-widgets/*` → `@dsfr-data/*` dans les workspaces
...
1. **Breaking change npm** : les utilisateurs existants de `gouv-widgets` devront migrer. Publier une version finale de `gouv-widgets` avec un message de deprecation pointant vers `dsfr-data`.
2. **URLs CDN** : `unpkg.com/gouv-widgets/...` et `jsdelivr.net/npm/gouv-widgets/...` ne fonctionneront plus. Documenter la migration.
