...
/**
 * <gouv-chart-a11y> - Companion d'accessibilite pour visualisations
 *
 ...
 * </gouv-dsfr-chart>
 * <gouv-chart-a11y for="mon-graph" source="data" table download
 *   description="L'Ile-de-France domine largement.">
 * </gouv-chart-a11y>
 */
@customElement('gouv-chart-a11y')
export class GouvChartA11y extends SourceSubscriberMixin(LitElement) {
  ...
  connectedCallback() {
    ...
    sendWidgetBeacon('gouv-chart-a11y');
    this._ensureId();
  ...
  private _ensureId() {
    if (!this.id) {
      this.id = `gouv-chart-a11y-${++autoIdCounter}`;
    }
  }
  ...
  render() {
    ...
    return html`
      <section class="gouv-chart-a11y"
        ...
        <p id="${descId}" class="gouv-chart-a11y__sr-only">
