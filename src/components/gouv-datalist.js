 ...
 * </gouv-datalist>
 */
let GouvDatalist = class GouvDatalist extends SourceSubscriberMixin(LitElement) {
    constructor() {
        super(...arguments);
        this.source = '';
    ...
    _handleFilter(key, e) {
        this._activeFilters = { ...this._activeFilters, [key]: e.target.value };
        this._currentPage = 1;
    }
    _handleSort(key) {
    ...
    _renderFilters(columns, filterableColumns) {
    ...
    _renderToolbar() {
        if (!this.recherche && !this.export?.includes('csv'))
            return '';
        return html `
    ...
    _renderTable(columns, paginatedData) {
        return html `
    ...
    _renderPagination(totalPages) {
        if (this.pagination <= 0 || totalPages <= 1)
            return '';
        const pages = [];
